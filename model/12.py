...
if __name__ == '__main__':
    ...
    with open(outputFile, 'w', newline='') as csvfile:
        ...
        for q_id in tqdm(queries):
            ...
            qry_bm25 = qry_bm25 \
                     + doc_bm25[tokey.index(ranks[0][0])] * 0.9 \
                     + doc_bm25[tokey.index(ranks[1][0])] * 0.8 \
                     + doc_bm25[tokey.index(ranks[2][0])] * 0.7 \
                     + doc_bm25[tokey.index(ranks[3][0])] * 0.6 \
                     + doc_bm25[tokey.index(ranks[4][0])] * 0.5 \
                     + doc_bm25[tokey.index(ranks[5][0])] * 0.4 \
                     + doc_bm25[tokey.index(ranks[6][0])] * 0.3 \
                     + doc_bm25[tokey.index(ranks[7][0])] * 0.2 \
                     + doc_bm25[tokey.index(ranks[8][0])] * 0.1 \
                     + doc_bm25[tokey.index(ranks[9][0])] * 0.1 \
