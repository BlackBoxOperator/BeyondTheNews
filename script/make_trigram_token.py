...
tokenFile = os.path.join('..', 'tokens', 'trigram_token.txt')
tokeyFile = os.path.join('..', 'tokens', 'trigram_tokey.txt')
ignoreFile = os.path.join('..', 'tokens', 'trigram_ignore.txt')
...
for index in tqdm(index2URL):
    ...
    tokey.write(index + '\n')
    token.write(' '.join([w for w in trigram(text.split())
