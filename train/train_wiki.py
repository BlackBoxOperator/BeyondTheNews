...
model = Word2Vec(tqdm(sentences, total=1023054), min_count=5, size=250, window=5, workers=3, callbacks=[EpochLogger()])

print("training model...")
#model.train(tqdm(sentences), total_examples=len(sentences), epochs=150, callbacks=[EpochLogger()])
model.train(tqdm(sentences, total=1023054), total_examples=model.corpus_count, epochs=150, callbacks=[EpochLogger()])
